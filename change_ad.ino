...
void setup() {
  ...
  if (button.begin(0x60, Wire1) == false) {
    Serial.println("Device did not acknowledge! Running scanner.");
  }
  if (button2.begin(0x61, Wire1) == false) {
    Serial.println("Device did not acknowledge! Running scanner.");
  }
  if (button3.begin(0x62, Wire1) == false) {
    Serial.println("Device did not acknowledge! Running scanner.");
  }
  if (button4.begin(0x63, Wire1) == false) {
    Serial.println("Device did not acknowledge! Running scanner.");
  }
   if (button5.begin(0x64, Wire1) == false) {
    Serial.println("Device did not acknowledge! Running scanner.");
  }
   if (button6.begin(0x65, Wire1) == false) {
    Serial.println("Device did not acknowledge! Running scanner.");
  }
  if (button7.begin(0x66, Wire1) == false) {
    Serial.println("Device did not acknowledge! Running scanner.");
  }
  if (button8.begin(0x67, Wire1) == false) {
    Serial.println("Device did not acknowledge! Running scanner.");
  }
    ...
    if (Serial.available()) {
      ...
      if (success) {
        if (newAddress > 0x08 && newAddress < 0x77) {
          ...
          if (button8.setI2Caddress(newAddress) == true) {
